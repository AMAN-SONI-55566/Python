...
# In Python, you can use comparison operators to compare strings. The equality == and the not equal to != operators are
# helpful when you need to search for a specific string in a body of text, a log file, a spreadsheet, a database, and more.
# You can also check user input strings to compare them to another string. Note that Boolean data types are not string data
# types (Boolean True is not equal to the string "True").  
...
# The comparison operators greater than > and less than < can be used to alphabetize words in Python. The letters of the 
# alphabet have numeric codes in Unicode (also known as ASCII values). The uppercase letters A to Z are represented by the
# Unicode values 65 to 90. The lowercase letters a to z are represented by the Unicode values 97 to 122. 

# To check if the first letter(s) of a string have a larger Unicode value (meaning the letter is closer to 122 or lowercase z) 
# than the first letter of another string, use the greater than operator: >

# To check if the first letter(s) of a string have a smaller Unicode value (meaning the letter is closer to 65 or uppercase A)
# than the first letter of another string, use the less than operator: < 

# Like numeric comparisons with the greater than > and less than < operators, comparisons between strings also return Boolean 
...
# The greater than or equal to >= and less than or equal to <= operators can be used with strings as well. Like the other comparison 
# operators, they will return a True or False Boolean result when a comparison is made between two strings. 

# To check if a string has a larger or equal Unicode value than the first letter(s) of another string, use the greater than or equal
# to operator: >= 

# To check if a string has a smaller or equal Unicode value than the first letter(s) of another string, use the less than or equal to
# operator: <=

# At this point, you should be familiar with how comparison operators work in Python. Can you determine what the results will be from 
# the comparisons listed below? When you are ready to check your answers, click Run.
...
# Use the Unicode chart to determine if the Unicode values of  the first letters of each string are higher, lower, or equal to one
# another. 
